...
public class TableDishView extends FrameLayout {
    ...
    private boolean botRight = false; // 人机桌：右侧固定 AIBOT 头像
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 兜底：反算失败/过小也保证桌面与座位可见，避免只剩桌号文字
        int head;
        if (headDp <= 0) {
            head = Math.max((int) (w * 0.20f), 1); // 至少组件宽的 20%
        } else {
            head = (int) (headDp * density);
        }
        int headGap = (int) (HEAD_TO_TABLE_DP * density);
        int desk = (int) (head * TABLE_RATIO);
        if (desk <= 0) desk = Math.max((int) (w * 0.5f), 1); // 兜底：桌面至少组件宽一半
        head = (int) (desk / TABLE_RATIO);
        int mid = w / 2;
    ...
    // 人机桌：右侧固定显示 AIBOT 头像（不随 rightPlayer 变化）
    public void setBotRight(boolean bot) {
        this.botRight = bot;
        refresh();
    }
